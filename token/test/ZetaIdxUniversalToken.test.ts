...
describe("ZetaIdxUniversalToken", function () {
    ...
    beforeEach(async function () {
        ...
        const ZetaIdxUniversalToken = await ethers.getContractFactory("ZetaIdxUniversalToken");
    ...
    describe("Wrap", function () {
        it("should wrap tokens correctly", async function () {
            ...
            for (let i = 0; i < testTokens.length; i++) {
                ...
                await testTokens[i].connect(user).approve(indexToken.address, tokenAmount);
            }
            ...
            for (let i = 0; i < testTokens.length; i++) {
                const tokenAmount = wrapAmount.mul(RATIOS[i]).div(100);
                expect(await testTokens[i].balanceOf(indexToken.address)).to.equal(tokenAmount);
            }
        });
