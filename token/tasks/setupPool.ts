...
task("setupPool", "Set up Uniswap liquidity pool for index token")
    ...
    .setAction(async (taskArgs: any, hre: any) => {
        ...
        const token1 = await hre.ethers.getContractAt("TestToken", taskArgs.testToken1);
        const token2 = await hre.ethers.getContractAt("TestToken", taskArgs.testToken2);
        const token3 = await hre.ethers.getContractAt("TestToken", taskArgs.testToken3);
        ...
        if (indexBalance.lt(indexAmount)) {
            ...
            await token1.approve(taskArgs.indexToken, token1Amount, { gasLimit: 100000 });
            await token2.approve(taskArgs.indexToken, token2Amount, { gasLimit: 100000 });
            await token3.approve(taskArgs.indexToken, token3Amount, { gasLimit: 100000 });
        ...
        // Create pair first
        const [pairToken0, pairToken1] = taskArgs.indexToken < taskArgs.wzeta 
        ...
        const indexApproveTx = await indexToken.approve(router.address, indexAmount, {
            gasLimit: 100000
        });
        ...
        // Get pool address
        const pairAddress = await factory.getPair(wzeta.address, indexToken.address);
